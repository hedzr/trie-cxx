...
void test_traits_is_duration() {
	static_assert(trie::chrono::is_duration<std::chrono::milliseconds>::value);
	static_assert(trie::chrono::is_duration_v<std::chrono::milliseconds>);

	static_assert(trie::chrono::is_duration_v<std::thread::id> == false);
}
...
void test_cpp_style(std::chrono::system_clock::time_point &now, std::tm &now_tm) {
	//trie::process::exec date_cmd("date  +\"%T,%N\"");
	{
...
void test_time_now() {
	struct timeval tv = trie::chrono::get_system_clock_in_us();
	std::chrono::system_clock::time_point now = trie::chrono::now();
	auto now1 = trie::chrono::clock::now();
...
void test_try_parse_by() {
	...
	for (auto &time_str : {
		...
		if (trie::chrono::try_parse_by(tm, time_str, "%H:%M:%S", "%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M:%S")) {
			tp = trie::chrono::tm_2_time_point(&tm);
			log_print("  - time '%s' parsed: tp = %s",
			          time_str,
			          // _twl.size(), hit, loop,
			          // trie::chrono::format_duration(d).c_str(),
			          // trie::chrono::format_time_point(tp).c_str(),
			          trie::chrono::format_time_point_to_local(tp).c_str());
		}
	}
}

void test_last_day_at_this_month() {
	namespace chr = trie::chrono;
	using clock = std::chrono::system_clock;
...
#define NOW_CASE(now_str, expected_str, desc, ofs) \
	...
	for (auto const &t : {
		...
		log_print("%40s: %s -> %s", t.desc, chr::format_time_point_to_local(now).c_str(),
		          chr::format_time_point_to_local(pt).c_str());

		auto tmp = t.expected;
		if (!chr::duration_is_zero(tmp)) {
			if (chr::compare_date_part(pt, tmp) != 0) {
				log_print("%40s: ERROR: expecting %s but got %s", " ", chr::format_time_point_to_local(tmp).c_str(),
				          chr::format_time_point_to_local(pt).c_str());
				exit(-1);
			}
		}
