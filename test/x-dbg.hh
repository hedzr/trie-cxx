...
#ifndef TRIE_CXX_X_DBG_HH
#define TRIE_CXX_X_DBG_HH
...
#error "Unsupported compiler"
   ...
   * @code{c++}
   * std::cout << trie::debug::type_name&lt;std::string>() << '\n';
   * @endcode
   *
   * 2. wrap the string_view into a std::string:
   * @code{c++}
   * std::cout &lt;&lt; std::string(trie::debug::type_name&lt;std::string>()) &lt;&lt; '\n';
   * printf(">>> %s\n", std::string(trie::debug::type_name&lt;std::string>()).c_str());
   * @endcode
   *
   */
	template<typename T>
	...
	static inline void print_stacktrace(FILE *out = stderr, int skip = 1) {
		...
		for (int i = 1 + skip; i < addrlen; i++) {
			...
			if (end_offset == nullptr) {
				...
				// clang: "0   test-app2-c2        0x000000010541a2cc _ZN4diff5debugL16print_stacktraceILi63EEEvP7__sFILE + 76"
				for (char *p = symbollist[i]; *p; ++p) {
...
namespace trie::exception {

	class trie_exception : public std::runtime_error {
	...
	public:
		trie_exception(const char *file, int line, const std::string &arg)
		    : std::runtime_error(arg) {
			std::ostringstream o;
			o << arg << "  " << file << ":" << line;
			msg = o.str();
#if !OS_WIN
			auto v = trie::debug::save_stacktrace(2);
			st.swap(v);
#endif
		}
		~trie_exception() noexcept override = default;
...
#define trie_throw_line(arg) throw trie::exception::trie_exception(__FILE__, __LINE__, arg)

	class trie_biz_error : public trie_exception {
	public:
		using trie_exception::trie_exception;
	};

	class required_flag_missed : public trie_biz_error {
	public:
		using trie_biz_error::trie_biz_error;
	};


	class dup_error : public trie_biz_error {
	public:
		using trie_biz_error::trie_biz_error;
	};

#define trie_throw_as(typ, ...) throw trie::exception::typ(__FILE__, __LINE__, __VA_ARGS__)
...
namespace trie::debug {
	...
	inline void dump_stacktrace(
		...
		std::cerr << '\n';
		if (print_stack) {
			if (depth == 0 && "trie::exception::trie_exception" == type(e)) {
#if !OS_WIN
				print_stacktrace(((trie::exception::trie_exception const *) (&e))->stacktrace());
#endif
			}
			try {
...
#define DIFF_DUMP_STACK_TRACE(e)         trie::debug::dump_stacktrace(__FILE__, __LINE__, e)
#define DIFF_DUMP_WITHOUT_STACK_TRACE(e) trie::debug::dump_stacktrace(__FILE__, __LINE__, e, false)
